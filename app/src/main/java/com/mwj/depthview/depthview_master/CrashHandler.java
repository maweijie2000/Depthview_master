...
public class CrashHandler implements UncaughtExceptionHandler {
	 ...
	 * 初始化
	 * @param context
	 * @param mCachePath:异常的补货保存的位置
	 */
	public void init(Context context,String mCachePath) {
		mContext = context;
	...
	private String getFilePath() {
		...
		if (isSDCardExist && isRootDirExist) {
			file_dir = Environment.getExternalStorageDirectory().getAbsolutePath() + cachePath;
		} else {
			// MyApplication.getInstance().getFilesDir()返回的路劲为/data/data/PACKAGE_NAME/files，其中的包就是我们建立的主Activity所在的包
			file_dir = mContext.getFilesDir().getAbsolutePath() + cachePath;
		}
		return file_dir;
	}
	/**
